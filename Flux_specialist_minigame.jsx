...
interface FluxSpecialistMinigameProps {
...
const FluxSpecialistMinigame: React.FC<FluxSpecialistMinigameProps> = ({
  reactorEnergy = 0.5,
  shipHealth = 1,
}) => {
  ...
  const stabilizedSpikesRef = useRef(0);
  const missedSpikesRef = useRef(0);
  ...
  useEffect(() => {
    ...
    const step = (now: number) => {
      ...
      if (remaining <= 0) {
        ...
        const ratio = stabilized / safeTotal;
        const pct = Math.round(ratio * 100);
...
export default FluxSpecialistMinigame;
